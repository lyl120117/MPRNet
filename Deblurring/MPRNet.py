...
def save(path, x):
    ...
    if not os.path.exists(root):
        os.makedirs(root)
    np.save(os.path.join(root, path), x.numpy())

def save_model(model, name):
    state_dict = {}
    for k, v in model.state_dict().items():
        state_dict[k] = v.numpy()
    pickle.dump(state_dict, open(name, 'wb'))

if __name__ == '__main__':
    mprnet = MPRNet()
    state_dicts = mprnet.state_dict()
    # print('state_dicts:', state_dicts.keys())
    parms = pickle.load(open('/home/ps/share/github/ml/pytorch/MPRNet/Deblurring/model.params', 'rb'))
    mprnet.load_dict(parms)


    # input_x = paddle.to_tensor(np.random.randn(1, 3, 256, 256), dtype='float32')
    input_x = paddle.to_tensor(np.load('/home/ps/share/github/ml/paddle/MPRNet/Deblurring/input_x.npy'), dtype='float32')
    print('input_x:', input_x.shape)
    img_stage3, img_stage2, img_stage1 = mprnet(input_x)
    print('img_stage1:', img_stage1.shape)
    print('img_stage1:', img_stage1.numpy())
    print('img_stage2:', img_stage2.shape)
    print('img_stage3:', img_stage3.shape)
    # save('input_x.npy', input_x)
    save('img_stage1.npy', img_stage1)
    save('img_stage2.npy', img_stage2)
    save('img_stage3.npy', img_stage3)
    print_network(mprnet, 'MPRNet')
